...
def _build_router() -> APIRouter:
    ...
    @router.get("/token", response_model=TokenMetadataResponse, tags=["token"])
    def token_metadata() -> TokenMetadataResponse:
        return TokenMetadataResponse.model_validate(asdict(TokenMetadataService().get_metadata()))

    @router.get("/networks", response_model=list[NetworkResponse], tags=["token"])
    def networks() -> list[NetworkResponse]:
        return [
            NetworkResponse.model_validate(asdict(network))
            for network in TokenMetadataService().list_networks()
        ]
    ...
    def quote(
        ...
        return QuoteResponse.model_validate(_decimal_safe(asdict(response)))
